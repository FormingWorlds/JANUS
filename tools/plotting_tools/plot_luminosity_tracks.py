...
for lum_track in reversed(lum_tracks):
    ...
    ages = luminosity_df['age'] * 1e3
    luminosities = luminosity_df['lum']

    p = plt.plot(ages, luminosities, label=star_mass + r' $M_{\odot}$')
    ...
    if star_mass == '1.0':
        ...
        plt.annotate(
            'Current Sun',
            xy=(4603, interpolate_luminosity([4603])),
            xycoords='data',
            xytext=(0, -10),
...
plt.ylabel(r'Bolometric luminosity ($\mathcal{L}/\mathcal{L}_{\odot}$)')
plt.xlabel('Time (Myr)')
