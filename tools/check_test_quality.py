#!/usr/bin/env python3
"""AST-based test-quality linter for the JANUS test suite.

Enforces the rules in `.github/.claude/rules/janus-tests.md` (sections 1 + 7):
...
- Forbidden weak assertions when they stand alone as the sole meaningful
  ...
  three-class discrimination guard from janus-tests.md section 2 uses
...
- Optional dependencies (``hypothesis``, ``mors``) imported at module top
...
* ``--reference-pinned-status``  Print the physics source files that lack at
  least one ``@pytest.mark.reference_pinned`` test in their companion test
  file. Does not exit non-zero on its own (advisory).
...
# Source rule: janus-tests.md section 5.
OPTIONAL_DEPS = {
    'hypothesis',
    'mors',
}

# Physics source files, keyed by their path relative to ``src/janus/`` and
# mapped to the companion test path relative to the repository root. Each
# source must have at least one @pytest.mark.physics_invariant test and at
# least one @pytest.mark.reference_pinned test in its companion test file
# (the rule in janus-tests.md section 12).
PHYSICS_SOURCES = {
    'utils/phys.py': 'tests/utils/test_phys.py',
    'utils/height.py': 'tests/utils/test_height.py',
    'utils/cp_funcs.py': 'tests/utils/test_cp_funcs.py',
    'modules/spectral_planck_surface.py': 'tests/modules/test_spectral_planck_surface.py',
    'modules/moist_adjustment_H2O.py': 'tests/modules/test_moist_adjustment_H2O.py',
    'modules/dry_adiabat_setup.py': 'tests/modules/test_dry_adiabat_setup.py',
}

# Utility sources are exempt from the physics-invariant / reference-pinned
# requirement but still subject to the anti-happy-path rules. Informational.
UTILITY_SOURCES = {
    '__init__.py',
    '_version.py',
    'set_socrates_env.py',
...
def reference_pinned_status() -> list[str]:
    ...
    For each source in PHYSICS_SOURCES, resolve the companion test file from
    the mapping value and check whether it exists and contains at least one
    ``@pytest.mark.reference_pinned`` decoration. Sources without such a test
    are reported. Missing companion test files are reported, not treated as
    an error.
    """
    missing = []
    for source, test_rel in sorted(PHYSICS_SOURCES.items()):
        ...
        if not test_path.exists():
            missing.append(f'{source} (no {test_rel})')
            continue
        if not _file_has_decorator(test_path, 'reference_pinned'):
            missing.append(f'{source} ({test_rel} has no @pytest.mark.reference_pinned)')
    return missing
...
def physics_invariant_status() -> list[str]:
    ...
    advisory, not blocking. Companion test files that do not exist yet are
    skipped.
    """
    flagged = []
...
def cmd_baseline() -> int:
    ...
    allow = os.environ.get('JANUS_TEST_QUALITY_ALLOW_REGRESS') == '1'
    if old and new_total > old_total and not allow:
        print(
            ...
            f'pre-existing violations), set JANUS_TEST_QUALITY_ALLOW_REGRESS=1.',
            file=sys.stderr,
        )
        return 2
