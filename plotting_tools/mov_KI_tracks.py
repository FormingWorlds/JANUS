...
def literature_comparison():
    ...
    ax5.plot(Goldblatt13_Ts, Goldblatt13_OLR, color=ga.vol_colors["qgray"], ls=":", lw=1.0, zorder=0.1)
    ax5.text(1900, 320, "Goldblatt+ 13", va="bottom", ha="right", fontsize=7, color=ga.vol_colors["qgray"], bbox=dict(fc='white', ec="white", alpha=0.5, pad=0.05, boxstyle='round'))
    # ax5.plot(Kopparapu13_Ts, Kopparapu13_OLR, color=ga.vol_colors["qgray"], ls="-.", lw=1.0, zorder=0.1)
    ax5.plot(Hamano15_Ts, Hamano15_OLR, color=ga.vol_colors["qgray"], ls="-.", lw=1.0, zorder=0.1)
    ax5.text(2180, 330, "Hamano+ 15", va="top", ha="left", fontsize=7, color=ga.vol_colors["qgray"], bbox=dict(fc='white', ec="white", alpha=0.5, pad=0.05, boxstyle='round'))
...
# Set up plot
fig, ((ax5, ax6), (ax3, ax4), (ax5, ax6)) = plt.subplots(2, 3, figsize=(18,10))
...
for setting in [ "trpp", "moist" ]: # "trpp", "moist", "tstep"
                ...
                for Mstar_idx, Mstar in enumerate(Mstar_range): 
                    ...
                    for tstar_idx, tstar in enumerate(star_age_range):
                        ...
                        if P_surf in prs_rangeA:
                            ...
                            l1, = ax5.plot(tmp_range, OLR_array, color=vol_color, ls=ls_list[prs_idx], lw=lw, label=ga.vol_latex[vol])
                            ...
                            if vol_idx == 0 and dist_idx == 0: 
                                l2, = ax5.plot([0],[0], color="gray", ls=ls_list[prs_idx], lw=lw, label=r"$P_\mathrm{s}$ = "+str(round(P_surf/1e+5))+" bar")
                        ...
                        if P_surf in prs_rangeB:
                            ...
                            l3, = ax6.plot(tmp_range, NET_array, color=vol_color, ls=ls_list[dist_idx], lw=lw, label=ga.vol_latex[vol])
                            
                            # Fill color and P_surf legends each only once
                            if dist_idx == 0: 
                                legendB1_handles.append(l3)
                            if Mstar_idx == 0 and vol_idx == 0:
                                l4, = ax6.plot([0],[0], color=ga.vol_colors["qgray"], ls=ls_list[dist_idx], lw=lw, label=r"$a$ = "+str(dist)+" au, $P_\mathrm{s}$ = "+str(round(P_surf/1e+5))+" bar")
    ...
    legendA1 = ax5.legend(handles=legendA1_handles, loc=2, ncol=2, fontsize=legend_fs)
    ax5.add_artist(legendA1)
    # Legend for the line styles
    legendA2 = ax5.legend(handles=legendA2_handles, loc=4, ncol=1, fontsize=legend_fs)

    ax5.set_xlabel(r'Surface temperature, $T_\mathrm{s}$ (K)', fontsize=label_fs)
    ax5.set_ylabel(r'Outgoing longwave radiation, $F^{\uparrow}_\mathrm{OLR}$ (W m$^{-2}$)', fontsize=label_fs)
    ax5.set_yscale("log")
    ax5.set_xlim(left=np.min(tmp_range), right=np.max(tmp_range))
    ax5.set_ylim(bottom=1e-4, top=3e+6)
    ax5.set_xticks([np.min(tmp_range), 500, 1000, 1500, 2000, 2500, np.max(tmp_range)])
    # ax5.set_yticks([1e-10, 1e-5, 1e0, 1e5])
    ax5.tick_params(axis='both', which='major', labelsize=ticks_fs)
    ax5.tick_params(axis='both', which='minor', labelsize=ticks_fs)

    ##### PLOT B settings
    # Legend for the main volatiles
    legendB1 = ax6.legend(handles=legendB1_handles, loc=2, ncol=2, fontsize=legend_fs)
    ax6.add_artist(legendB1)
    # Legend for the line styles
    legendB2 = ax6.legend(handles=legendB2_handles, loc=4, ncol=1, fontsize=legend_fs)

    ax6.set_xlabel(r'Surface temperature, $T_\mathrm{s}$ (K)', fontsize=label_fs)
    ax6.set_ylabel(r'Net outgoing radiation, $F^{\uparrow}_\mathrm{net}$ (W m$^{-2}$)', fontsize=label_fs)
    ax6.set_yscale("symlog")
    ax6.set_xlim(left=np.min(tmp_range), right=np.max(tmp_range))
    ax6.set_ylim(bottom=1e-4, top=3e+6)
    ax6.set_xticks([np.min(tmp_range), 500, 1000, 1500, 2000, 2500, np.max(tmp_range)])
    ax6.tick_params(axis='both', which='major', labelsize=ticks_fs)
    ax6.tick_params(axis='both', which='minor', labelsize=ticks_fs)

    # Annotate subplot numbering
    ax5.text(0.98, 0.985, 'A', color="k", rotation=0, ha="right", va="top", fontsize=20, transform=ax5.transAxes, bbox=dict(fc='white', ec="white", alpha=0.01, pad=0.1, boxstyle='round'))
    ax6.text(0.98, 0.985, 'B', color="k", rotation=0, ha="right", va="top", fontsize=20, transform=ax6.transAxes, bbox=dict(fc='white', ec="white", alpha=0.01, pad=0.1, boxstyle='round'))

    # Indicate cooling/heating regimes
    ax6.fill_between(tmp_range, 0, +1e+10, alpha=0.05, color="blue")
    ax6.text(np.max(tmp_range)*0.99, 0.3, "Net cooling", va="bottom", ha="right", fontsize=legend_fs, color=ga.vol_colors["qblue_dark"])
    ax6.text(np.max(tmp_range)*0.99, -0.3, "Net heating", va="top", ha="right", fontsize=legend_fs, color=ga.vol_colors["qred_dark"])
    ax6.fill_between(tmp_range, 0, -1e+10, alpha=0.05, color="red")
