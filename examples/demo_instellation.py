...
if __name__ == '__main__':
    ...
    StellarSpectrum.InsertStellarSpectrum(
        str(FWL_DATA_DIR / 'spectral_files' / 'Oak' / '318' / 'Oak.sf'), socstar, dirs['output']
    )
    ...
    for i in range(7):
        ...
        atm = MCPA_CBL(
            dirs, atm, False, rscatter=True, T_surf_max=9.0e99, T_surf_guess=atm.trppT + 100
        )
    ...
    ax1.plot(
        r_arr, asf_arr, color='royalblue', lw=2.5, label='ASF'
    )  # absorbed stellar flux (sw down)
    ax1.plot(
        r_arr, OLR_arr, color='crimson', lw=2.5, label='OLR'
    )  # outgoing longwave radiation (lw up)
    ...
    # Planet data
    x_planets = {'Earth': 1.0, 'Venus': 0.723, 'Mercury': 0.387}
    c_planets = {'Earth': 'deepskyblue', 'Venus': 'goldenrod', 'Mercury': 'violet'}
    for p in x_planets.keys():
        for ax in (ax1, ax2):
            ax.axvline(
                x=x_planets[p], color=c_planets[p], lw=3, linestyle='dashed', label=p, zorder=1
            )

    arr_magma = np.ones(len(r_arr)) * T_magma
    ax2.plot(
        r_arr, np.zeros(len(r_arr)), zorder=6, color='silver', lw=2.5, label=r'$\tilde{T}_s$'
    )  # magma temperature
    ax2.plot(
        r_arr, ts_arr - arr_magma, zorder=6, color='black', lw=2.5, label=r'$T_s$'
    )  # surface solution temperature
