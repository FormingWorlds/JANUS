...
def ncout_surf(file, lon, lat, basis, alb):
    ...
    if nvals == 1:
        ...
        raise RuntimeError(
            ' Error in ncout_surf: arrays dont match', nvals, n_lon * n_lat * levels
        )
    ...
    write_dim(ncdf_file, levels, basis, 'basis', 'i2', 'basis', 'None', 'BASIS FUNCTION')

    write_var(ncdf_file, albs, 'alb', 'f4', ('basis', 'lat', 'lon'), 'None', 'ALBEDO WEIGHTS')
...
def ncout_spectral_surf(file, lon, lat, bands, alb):
    ...
    if nvals == 1:
        ...
        raise RuntimeError(
            ' Error in ncout_surf: arrays dont match', nvals, n_lon * n_lat * bands
        )

    ncdf_file = create_cdf(file)
    write_dim(ncdf_file, n_lon, lon, 'lon', 'f4', 'lon', 'degree', 'LONGITUDE')
    write_dim(ncdf_file, n_lat, lat, 'lat', 'f4', 'lat', 'degree', 'LATITUDE')
    write_dim(ncdf_file, 1, basis, 'basis', 'i2', 'basis', 'None', 'BASIS FUNCTION')
    write_dim(
        ncdf_file, np.sum(bands), np.arange(bands) + 1, 'bands', 'i2', 'bands', 'None', 'BANDS'
    )

    write_var(
        ncdf_file, albs, 'alb', 'f4', ('bands', 'basis', 'lat', 'lon'), 'None', 'ALBEDO WEIGHTS'
    )
...
def ncout2d(file, lon, lat, val, name=None, longname=None, units=None):
    ...
    if nvals == 1:
        ...
        raise RuntimeError(' Error in ncout2d: arrays dont match', nvals, n_lon * n_lat)
...
def ncout3d(file, lon, lat, p, val, name=None, longname=None, units=None):
    ...
    if nvals == 1:
        ...
        raise RuntimeError(
            ' Error in ncout3d: arrays dont match', nvals, n_lon * n_lat * levels
        )
...
def ncout_opt_prop(file, lon, lat, p, bands, absp, scat, phf):
    ...
    if nvals == levels * bands:
        ...
        absp_vals = np.zeros(n_lon * n_lat * levels * bands).reshape(
            bands, levels, n_lat, n_lon
        )
        ...
        raise RuntimeError(
            ' Error in ncout_opt_prop: absp arrays dont match',
            nvals,
            n_lon * n_lat * levels * bands,
        )

    nvals = np.size(scat)
    if nvals == levels * bands:
        scat.reshape(bands, levels)
        scat_vals = np.zeros(n_lon * n_lat * levels * bands).reshape(
            bands, levels, n_lat, n_lon
        )
        for i in np.arange(n_lon):
            for j in np.arange(n_lat):
        ...
        raise RuntimeError(
            ' Error in ncout_opt_prop: scat arrays dont match',
            nvals,
            n_lon * n_lat * levels * bands,
        )
    ...
    nvals = np.size(phf)
    if nvals == 1:
        phf_vals = np.zeros(n_lon * n_lat * levels * bands).reshape(
            bands, 1, levels, n_lat, n_lon
        )
        phf_vals += np.sum(phf)
        ...
        raise RuntimeError(
            ' Error in ncout_opt_prop: phf arrays dont match',
            nvals,
            n_lon * n_lat * levels * bands,
        )
    ...
    write_dim(ncdf_file, bands, np.arange(bands) + 1, 'band', 'i2', 'band', 'none', 'band')

    write_var(
        ncdf_file, absp_vals, 'abs', 'f4', ('band', 'plev', 'lat', 'lon'), 'M-1', 'absorption'
    )
    write_var(
        ncdf_file, scat_vals, 'scat', 'f4', ('band', 'plev', 'lat', 'lon'), 'M-1', 'scattering'
    )
    write_var(
        ncdf_file,
        phf_vals,
