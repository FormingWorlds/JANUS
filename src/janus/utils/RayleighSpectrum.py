#!/usr/bin/env python3
"""
Created on Tue Jul 14 13:44:49 2020
...
def species_info(species):
    ...
    if species.lower() == 'co2':
        ...
        info_dict['cross_section'] = (
            0.3743 * cross_section_hydrogen_1um
        )  # m^2/kg; mass weighted rayleigh cross section of water, from same table as h2
        ...
        info_dict['cross_section'] = (
            cross_section_cm2_molecule * 10**-4 * 6.022e23 / 18e-3
        )  # converting to m^2/kg
        ...
        info_dict['cross_section'] = (
            cross_section_cm2_molecule * 10**-4 * 6.022e23 / 44e-3
        )  # converting to m^2/kg
...
def cross_section(wavelength, info_dict):  # wavelength in m
    if 'Delta' in info_dict:
        ...
        coefficient = cross_section_1um * normalization_wavelength**4.0 / wavelength**4
    return coefficient
...
def band_integrator(species_list, molar_mixing_ratio_list, wavelength1_list, wavelength2_list):
    ...
    for n, species in enumerate(species_list):
        ...
        partial_coefficient = np.vectorize(spint.quad, excluded=['args'])(
            cross_section, wavelength1_list, wavelength2_list, args=info_tup
        )[0]
...
def rayleigh_coeff_adder(
    ...
    spectral_file = open(spectral_file_path)

    # Load a dummy file that will hold the wavelength bands

    wavelength_band_file = open(wavelength_dummy_file_path, 'w')
    ...
    cross_section_integral = band_integrator(
        species_list, mixing_ratio_list, wavelength_bands[:, 1], wavelength_bands[:, 2]
    )
