...
def radCompSoc(
    ...
    if not atm.bands_set:
        raise Exception(
            'Cannot run radiative transfer because bands have not been loaded into atmos object.'
        )
    ...
    if rscatter == True:
        ...
        if (not os.path.exists(runspectral_file)) or rewrite_gas:
            ...
            RayleighSpectrum.rayleigh_coeff_adder(
                species_list=rscatter_snames,
    ...
    fthis = basename + '.surf'
    if check_cfg(fthis):
        nctools.ncout_surf(fthis, 0, 0, 1, float(atm.albedo_s))

    fthis = basename + '.tstar'
    if check_cfg(fthis) or check_tmp(fthis):
        nctools.ncout2d(fthis, 0, 0, atm.ts, 'tstar', longname='Surface Temperature', units='K')

    fthis = basename + '.pstar'
    if check_cfg(fthis):
        nctools.ncout2d(fthis, 0, 0, atm.ps, 'pstar', longname='Surface Pressure', units='PA')
    ...
    fthis = basename + '.stoa'
    toah = (
        atm.instellation * (1.0 - atm.albedo_pl) * atm.inst_sf
    )  # Values passed to socrates should not include cos(theta), since it's applied during the RT
    atm.toa_heating = toah * np.cos(atm.zenith_angle * np.pi / 180.0)
    if check_cfg(fthis):
        nctools.ncout2d(
            fthis, 0, 0, toah, 'stoa', longname='Solar Irradiance at TOA', units='WM-2'
        )

    # T, P + volatiles
    fthis = basename + '.t'
    if check_tmp(fthis):
        nctools.ncout3d(fthis, 0, 0, atm.p, atm.tmp, 't', longname='Temperature', units='K')

    fthis = basename + '.tl'
    if check_tmp(fthis):
        nctools.ncout3d(fthis, 0, 0, atm.pl, atm.tmpl, 'tl', longname='Temperature', units='K')

    fthis = basename + '.p'
    if check_tmp(fthis):
        nctools.ncout3d(fthis, 0, 0, atm.p, atm.p, 'p', longname='Pressure', units='PA')
    ...
    if check_gas(fthis):
        nctools.ncout3d(
            ...
            0,
            atm.p,
            phys.molar_mass['H2O'] / atm.mu * atm.x_gas['H2O'],
            'q',
            longname='q',
            units='kg/kg',
        )
    ...
    if atm.do_cloud:
        ...
        if check_tmp(fthis):
            nctools.ncout3d(
                fthis, 0, 0, atm.p, atm.re, 're', longname='Effective Radius', units='M'
            )
        ...
        if check_tmp(fthis):
            nctools.ncout3d(
                ...
                atm.lwm,
                'lwm',
                longname='Liquid Water Mass Fraction',
                units='GG-1',
            )
        ...
        if check_tmp(fthis):
            nctools.ncout3d(
                fthis, 0, 0, atm.p, atm.clfr, 'clfr', longname='Cloud Fraction', units='NONE'
            )
    ...
    if atm.do_cloud:
        seq_sw_ex = [
            ...
            ' -S -g ' + overlap_flag,
            ' -C ',
            str(atm.cloud_scheme),
            ' -K ',
            str(atm.cloud_representation),
            ' -d ',
            str(atm.droplet_type),
            ' -v ',
            str(atm.solver),
            ' -u',
            scatter_flag,
    ...
    if socrates_use_namelist:
        ...
        for vol in atm.vol_list.keys():
            mmw += (
                np.mean(atm.x_gas[vol]) * phys.molar_mass[vol]
            )  # There's probably a more accurate way to handle this.
    ...
    atm.SW_flux_net = np.squeeze(
        np.sum(uflxsw[:, :], axis=0)[:, 0, 0] - np.sum(vflxsw[:, :], axis=0)[:, 0, 0]
    )
    atm.LW_flux_net = np.squeeze(
        np.sum(uflxlw[:, :], axis=0)[:, 0, 0] - np.sum(dflxlw[:, :], axis=0)[:, 0, 0]
    )
    ...
    atm.flux_up_total = np.squeeze(
        np.sum(uflxlw[:, :], axis=0)[:, 0, 0] + np.sum(uflxsw[:, :], axis=0)[:, 0, 0]
    )
    atm.flux_down_total = np.squeeze(
        np.sum(vflxsw[:, :], axis=0)[:, 0, 0] + np.sum(dflxlw[:, :], axis=0)[:, 0, 0]
    )

    # Total net flux (W/m^2)
    atm.net_flux = np.squeeze(
        np.sum(uflxlw[:, :], axis=0)[:, 0, 0]
        - np.sum(dflxlw[:, :], axis=0)[:, 0, 0]
        + np.sum(uflxsw[:, :], axis=0)[:, 0, 0]
        - np.sum(vflxsw[:, :], axis=0)[:, 0, 0]
    )

    # Total net flux per band (W/m^2)
    atm.net_spectral_flux = (
        uflxlw[:, :, 0, 0] + uflxsw[:, :, 0, 0] - dflxlw[:, :, 0, 0] - vflxsw[:, :, 0, 0]
    )
    ...
    if recalc == False:
        ...
        # Total heating (K/day)
        atm.net_heating = np.squeeze(
            np.sum(hrtssw[:, :], axis=0) + np.sum(hrtslw[:, :], axis=0)
        )
