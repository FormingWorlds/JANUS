...
def atm_z(atm, idx):
    ...
    atm.rho[idx] = (
        ...
        / phys.R_gas
        / atm.tmp[idx]
        * (atm.mu[idx] + mu_c * atm.alpha_cloud * atm.xc[idx] / (atm.xv[idx] + atm.xd[idx]))
    )
    #     cell centres
    ...
    # Next gravity
    atm.grav_z[idx + 1] = (
        atm.grav_s * ((atm.planet_radius) ** 2) / ((atm.planet_radius + atm.z[idx + 1]) ** 2)
    )
...
def Tdew(switch, p):
    ...
    if switch == 'H2O':
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_H2O) * math.log(p / pref))
    ...
    if switch == 'CH4':
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_CH4) * math.log(p / pref))
    if switch == 'CO2':
        p_triple = 5.11e5
        if p > p_triple:
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_CO2) * math.log(p / pref))
    if switch == 'CO':
        Tref = (
            100.0  # K, arbitrary point (100K,esat(100K)=4.6bar) on the coexistence curve of CO
        )
        pref = p_sat('CO', Tref)
        L_CO = phys.CO.L_vaporization * phys.co.MolecularWeight * 1e-3
        return Tref / (1.0 - (Tref * phys.R_gas / L_CO) * math.log(p / pref))
    if switch == 'N2':
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_N2) * math.log(p / pref))
    if switch == 'O2':
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_O2) * math.log(p / pref))
    if switch == 'O3':
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_O3) * math.log(p / pref))
    if switch == 'H2':
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_H2) * math.log(p / pref))
    if switch == 'He':
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_He) * math.log(p / pref))
    if switch == 'NH3':
        ...
        return Tref / (1.0 - (Tref * phys.R_gas / L_NH3) * math.log(p / pref))
...
def dlnT_dlnP_d(lnP, lnT, atm):
    ...
    for vol in atm.vol_list.keys():
        ...
        if (
            np.isclose(atm.p_vol[vol][idx], p_sat(vol, tmp, water_lookup=atm.water_lookup))
            or atm.p_vol[vol][idx] > p_sat(vol, tmp, water_lookup=atm.water_lookup)
        ) and tmp < get_T_crit(vol):
        ...
        # sums for subsaturated comps
            ...
            denom_sum += (
                cpv(vol, tmp) * eta_vol
            )  # The eta_vol (x_vol/x_d) is there so that if there are multiple dry components the average dry specific heat is used
...
def condensation(atm, idx, wet_list, dry_list, prs_reset):
    ...
    if idx == 0:
        ...
        if sum(atm.vol_list.values()) != 1.0:
            vol_list_new = {}
            for vol in atm.vol_list.keys():
    ...
    for vol in atm.vol_list.keys():
        ...
        if atm.p_vol[vol][idx] < p_sat(vol, tmp, water_lookup=atm.water_lookup):
            ...
            if idx == 0:
                ...
                # mu_old = atm.mu[idx-1]
                atm.x_cond[vol][idx] = (
                    atm.x_cond[vol][idx - 1]
                    + atm.x_gas[vol][idx - 1]
                    - (1 - atm.xc[idx - 1]) * (atm.p_vol[vol][idx] / atm.p[idx])
                )
    ...
    for vol in atm.vol_list.keys():
        ...
        atm.cp[idx] += (
            atm.x_gas[vol][idx] * cpv(vol, atm.tmp[idx])
            + atm.x_cond[vol][idx] * cp_cond(vol, atm.tmp[idx]) * atm.alpha_cloud
        )
    ...
    for vol in atm.vol_list.keys():
        ...
        if atm.p_vol[vol][idx] >= p_sat(vol, tmp, water_lookup=atm.water_lookup):
            ...
            if atm.vol_list[vol] > 0.0 and vol not in dry_list and vol not in wet_list:
                dry_list.append(vol)
...
def general_adiabat(atm):
    ...
    for vol in atm.vol_list.keys():
        if np.isclose(
            atm.vol_list[vol] * atm.ps, p_sat(vol, atm.ts, water_lookup=atm.water_lookup)
        ):
            wet_list.append(vol)
...
def interpolate_atm(atm):
    ...
    prs_itp = (atm.pl[1:] + atm.pl[:-1]) / 2.0
    tmp_itp = (atm.tmpl[1:] + atm.tmpl[:-1]) / 2.0
...
def plot_adiabats(atm, filename='output/general_adiabat.pdf'):
    ...
    for vol in vol_list_sorted.keys():
        ...
        if atm.vol_list[vol] > 1e-10:
            ...
            ax1.semilogy(
                T_sat_array,
                Psat_array,
                label=r'$p_\mathrm{sat}$' + vol_latex[vol],
                lw=ls_ind,
                ls=':',
    ...
    ax1.semilogy(
        dry_adiabat(atm.ts, atm.pl, atm.cp[-1]),
        atm.pl,
        color=vol_colors['black_3'],
        ls='-.',
        lw=ls_dry,
        label=r'Dry adiabat',
    )  # Functional form
    ...
    ax1.semilogy(
        atm.tmpl,
        atm.pl,
        color=vol_colors['black_1'],
        lw=ls_moist,
        label='General adiabat',
        alpha=0.99,
    )
...
if __name__ == '__main__':
    ...
    atm = atmos(
        T_surf, P_surf, P_top, pl_radius, pl_mass, re, lwm, clfr, [], vol_mixing=vol_list
    )
