...
class atmos:
    def __init__(
        ...
        clfr: float = 0.0,
        albedo_s: float = 0.0,
        albedo_pl: float = 0.175,
        zenith_angle: float = 54.74,
    ):
        """Atmosphere class
        ...
        self.instellation = 0.0  # Instellation at planet's orbital separation, W/m^2
        self.toa_heating = 0.0  # ASF

        self.inst_sf = (
            3.0 / 8.0
        )  # Scale factor applied to instellation (see Cronin+14 for definitions)
        self.albedo_s = albedo_s  # surface albedo
        self.albedo_pl = (
            albedo_pl  # Bond albedo (scattering) applied to self.toa_heating in socrates.py
        )
        ...
        self.Rcp = 2.0 / 7.0  # standard earth air
        self.n_species = 16
        ...
        if self.bands_set:
            ...
            self.cff = np.zeros(
                [self.nbands, self.nlev_save]
            )  # W/m^2/(band). Compute by recompiling SOCRATES after setting the flags on lines 346 and 347 of src/aux/l_run_cdf.F90 to TRUE.
        ...
        if self.do_cloud:
            self.cloud_scheme = (
                2  # -C 2 = ip_cloud_mix_max : maximum/random overlap in a mixed column
            )
            self.cloud_representation = (
                1  # -K 1 = ip_cloud_homogen : ice and water mixed homogeneously
            )
    ...
    def from_file(
        ...
        alpha = cfg['atmos']['alpha'] if 'alpha' in cfg['atmos'] else 0.0
        trppT = cfg['atmos']['trppT'] if 'trppT' in cfg['atmos'] else 290.0
        do_cloud = cfg['atmos']['do_cloud'] if 'do_cloud' in cfg['atmos'] else False
        re = cfg['atmos']['re'] if 're' in cfg['atmos'] else 0.0
        lwm = cfg['atmos']['lwm'] if 'lwm' in cfg['atmos'] else 0.0
        clfr = cfg['atmos']['clfr'] if 'clfr' in cfg['atmos'] else 0.0
        albedo_pl = cfg['atmos']['albedo_pl'] if 'albedo_pl' in cfg['atmos'] else 0.175
        albedo_s = cfg['atmos']['albedo_s'] if 'albedo_s' in cfg['atmos'] else 0.0
    ...
    def setTropopauseTemperature(self):
        ...
        T_eqm = (self.instellation * self.inst_sf * (1.0 - self.albedo_pl) / phys.sigma) ** (
            1.0 / 4.0
        )
    ...
    def write_ncdf(self, fpath: str):
        ...
        self.z, self.zl, self.height_error = integrate_heights(
            self, self.planet_mass, self.planet_radius
        )
        ...
        # Scalar quantities
        #    Create variables
        var_tstar = ds.createVariable('tmp_surf', 'f4')
        var_tstar.units = 'K'  # Ground temperature
        var_trppP = ds.createVariable('trppP', 'f4')
        var_trppP.units = 'Pa'
        var_inst = ds.createVariable('instellation', 'f4')
        var_inst.units = 'W m-2'  # Solar flux at TOA
        var_s0fact = ds.createVariable(
            'inst_factor', 'f4'
        )  # Scale factor applied to instellation
        var_znth = ds.createVariable('zenith_angle', 'f4')
        var_znth.units = 'deg'  # Scale factor applied to instellation
        var_albbond = ds.createVariable(
            'bond_albedo', 'f4'
        )  # Bond albedo used to scale-down instellation
        var_toah = ds.createVariable('toa_heating', 'f4')
        var_toah.units = 'W m-2'  # TOA SW BC
        var_tmagma = ds.createVariable('tmagma', 'f4')
        var_tmagma.units = 'K'  # Magma temperature
        var_tmin = ds.createVariable('tfloor', 'f4')
        var_tmin.units = 'K'  # Minimum temperature
        var_tmax = ds.createVariable('tceiling', 'f4')
        var_tmax.units = 'K'  # Maximum temperature
        var_plrad = ds.createVariable('planet_radius', 'f4')
        var_plrad.units = 'm'  # Value taken for planet radius
        var_gsurf = ds.createVariable('surf_gravity', 'f4')
        var_gsurf.units = 'm s-2'  # Surface gravity
        var_albsurf = ds.createVariable('surf_albedo', 'f4')  # Surface albedo
        var_sknd = ds.createVariable('cond_skin_d', 'f4')
        var_sknd.units = 'm'  # Conductive skin thickness
        var_sknk = ds.createVariable('cond_skin_k', 'f4')
        var_sknk.units = 'W m-1 K-1'  # Conductive skin thermal conductivity
        var_zok = ds.createVariable('height_ok', 'S1')  # Hydrostatic integration is ok
        var_overlap = ds.createVariable(
            'overlap_type', int
        )  # gas overlap method (integer - rad_pcf.f90)
        ...
        # Layer quantities
        #    Create variables
        var_p = ds.createVariable('p', 'f4', dimensions=('nlev_c'))
        var_p.units = 'Pa'
        var_pl = ds.createVariable('pl', 'f4', dimensions=('nlev_l'))
        var_pl.units = 'Pa'
        var_tmp = ds.createVariable('tmp', 'f4', dimensions=('nlev_c'))
        var_tmp.units = 'K'
        var_tmpl = ds.createVariable('tmpl', 'f4', dimensions=('nlev_l'))
        var_tmpl.units = 'K'
        var_z = ds.createVariable('z', 'f4', dimensions=('nlev_c'))
        var_z.units = 'm'
        var_zl = ds.createVariable('zl', 'f4', dimensions=('nlev_l'))
        var_zl.units = 'm'
        var_grav = ds.createVariable('gravity', 'f4', dimensions=('nlev_c'))
        var_grav.units = 'm s-2'
        var_cp = ds.createVariable('cp', 'f4', dimensions=('nlev_c'))
        var_cp.units = 'J/(kg K)'

        var_mmw = ds.createVariable('mmw', 'f4', dimensions=('nlev_c'))
        var_mmw.units = 'kg mol-1'
        var_gases = ds.createVariable(
            'gases', 'S1', dimensions=('ngases', 'nchars')
        )  # Names of gases
        var_mr = ds.createVariable(
            'x_gas', 'f4', dimensions=('nlev_c', 'ngases')
        )  # Mixing ratios per level
        var_cmr = ds.createVariable(
            'x_cond', 'f4', dimensions=('nlev_c', 'ngases')
        )  # Condensate mixing ratios per level
        var_pvol = ds.createVariable('p_vol', 'f4', dimensions=('nlev_c', 'ngases'))
        var_pvol.units = 'Pa'  # Gas phase partial pressures
        var_plvol = ds.createVariable('pl_vol', 'f4', dimensions=('nlev_l', 'ngases'))
        var_pvol.units = 'Pa'
        if self.bands_set:
            var_fdl = ds.createVariable('fl_D_LW', 'f4', dimensions=('nlev_l'))
            var_fdl.units = 'W m-2'
            var_ful = ds.createVariable('fl_U_LW', 'f4', dimensions=('nlev_l'))
            var_ful.units = 'W m-2'
            var_fnl = ds.createVariable('fl_N_LW', 'f4', dimensions=('nlev_l'))
            var_fnl.units = 'W m-2'
            var_fds = ds.createVariable('fl_D_SW', 'f4', dimensions=('nlev_l'))
            var_fds.units = 'W m-2'
            var_fus = ds.createVariable('fl_U_SW', 'f4', dimensions=('nlev_l'))
            var_fus.units = 'W m-2'
            var_fns = ds.createVariable('fl_N_SW', 'f4', dimensions=('nlev_l'))
            var_fns.units = 'W m-2'
            var_fd = ds.createVariable('fl_D', 'f4', dimensions=('nlev_l'))
            var_fd.units = 'W m-2'
            var_fu = ds.createVariable('fl_U', 'f4', dimensions=('nlev_l'))
            var_fu.units = 'W m-2'
            var_fn = ds.createVariable('fl_N', 'f4', dimensions=('nlev_l'))
            var_fn.units = 'W m-2'
            var_hr = ds.createVariable('hrate', 'f4', dimensions=('nlev_c'))
            var_hr.units = 'K day-1'
            var_bul = ds.createVariable('ba_U_LW', 'f4', dimensions=('nlev_l', 'nbands'))
            var_bul.units = 'W m-2'
            var_bdl = ds.createVariable('ba_D_LW', 'f4', dimensions=('nlev_l', 'nbands'))
            var_bdl.units = 'W m-2'
            var_bnl = ds.createVariable('ba_N_LW', 'f4', dimensions=('nlev_l', 'nbands'))
            var_bnl.units = 'W m-2'
            var_bus = ds.createVariable('ba_U_SW', 'f4', dimensions=('nlev_l', 'nbands'))
            var_bus.units = 'W m-2'
            var_bds = ds.createVariable('ba_D_SW', 'f4', dimensions=('nlev_l', 'nbands'))
            var_bds.units = 'W m-2'
            var_bns = ds.createVariable('ba_N_SW', 'f4', dimensions=('nlev_l', 'nbands'))
            var_bns.units = 'W m-2'
            var_bmin = ds.createVariable('bandmin', 'f4', dimensions=('nbands'))
            var_bmin.units = 'm'
            var_bmax = ds.createVariable('bandmax', 'f4', dimensions=('nbands'))
            var_bmax.units = 'm'

        if self.has_contfunc:
            var_cff = ds.createVariable('contfunc', 'f4', dimensions=('nbands', 'nlev_c'))
            var_cff.units = 'W m-2 m-1'  ## units??

        var_re = ds.createVariable('re', 'f4', dimensions=('nlev_c'))
        var_re.units = 'm'
        var_lwm = ds.createVariable('lwm', 'f4', dimensions=('nlev_c'))
        var_lwm.units = 'kg kg-1'
        var_clfr = ds.createVariable('clfr', 'f4', dimensions=('nlev_c'))
        var_clfr.units = 'none'
        ...
        var_cmr[:] = np.array(
            [[self.x_cond[gas][i] for i in range(nlev_c - 1, -1, -1)] for gas in gas_list]
        ).T
        var_pvol[:] = np.array(
            [[self.p_vol[gas][i] for i in range(nlev_c - 1, -1, -1)] for gas in gas_list]
        ).T
        var_plvol[:] = np.array(
            [[self.pl_vol[gas][i] for i in range(nlev_l - 1, -1, -1)] for gas in gas_list]
        ).T
