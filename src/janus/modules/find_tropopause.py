...
def find_tropopause(atm_moist, dynamic: bool, verbose=True):
    ...
    if dynamic:
        ...
        trpp_idx = 0
        signchange = (
            (np.roll(np.sign(atm_moist.net_heating), 1) - np.sign(atm_moist.net_heating)) != 0
        ).astype(int)[1:]
        signchange_indices = np.nonzero(signchange)[0]

        # Criteria for "significant " heating
        DeltaT_max_sign = 50.0
        DeltaT_at_trpp = 30.0
        DeltaT_mean_sign = 10.0
        ...
        if np.size(signchange_indices) > 0 and np.max(atm_moist.net_heating) > DeltaT_max_sign:
            ...
            while (
                atm_moist.net_heating[max_heat_idx] > DeltaT_at_trpp
                and max_heat_idx < len(atm_moist.p) - 1
            ):
                max_heat_idx += 1
