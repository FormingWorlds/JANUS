...
def plot_emission(
    atm: atmos,
    filename: str = 'output/toa_emission.pdf',
    level_idx: int = 0,
    planck_surface: bool = True,
    incoming_solar: bool = True,
        ...
        for i in range(planck_samps):
            ...
            yp[i] = (
                ...
                * phys.c**2.0
                / wl**5.0
                * 1.0
                / (np.exp(phys.h * phys.c / (wl * phys.k * planck_tmp)) - 1.0)
            )
    ...
    wlwn = lambda x: (
        1.0e7 / (x - 1.0e-20)
    )  # wavelength <-> wavenumber, with small offset to prevent divergence
