...
def update_for_constant_RH(atm, R_h):
    ...
    atm.p = sum(
        atm.p_vol[molecule] for molecule in atm.vol_list.keys()
    )  # Updating the total pressure
    atm.ps = float(
        sum(atm.p_vol[molecule][-1] for molecule in atm.vol_list.keys())
    )  # Not forgetting the surface pressure
