...
def compute_dry_adiabat(
    ...
    dT_max = 20.0  # K, Maximum temperature change per radiation step
    T_floor = 10.0  # K, Temperature floor to prevent SOCRATES crash
    ...
    # Time stepping
    for i in range(rad_steps):
        # atm.toa_heating = 0

        # Compute radiation, midpoint method time stepping
        try:
            if atm.do_cloud:
                atm_dry = simple_cloud(
                    atm_dry
                )  # Before radiation, set up the cloud for Socrates using the current PT profile
            ...
            if surf_dt:
                ...
                net_Fs = (
                    atm_dry.SW_flux_down[len(atm_dry.tmp)]
                    + atm_dry.LW_flux_down[len(atm_dry.tmp)]
                    - atm_dry.LW_flux_up[len(atm_dry.tmp)]
                    - atm_dry.SW_flux_up[len(atm_dry.tmp)]
                )
                dT_surf += net_Fs / cp_surf
                dT_dry, dT_surf = simple_boundary_tend(
                    len(dT_dry) - 1,
                    atm_dry.tmp,
                    atm_dry.ts,
                    dT_dry,
            ...
            if pure_steam_adj:
                dT_moist = moist_adj(atm_dry, 1000.0)
            ...
            if i % 2 == 1 and standalone == True:
                ...
                print(
                    '\tT_top = ' + str(atm_dry.tmp[0]) + ' K,',
                    'dT_top = ' + str(dTtop_dry) + ' K',
                )
                print(
                    '\tT_bot = ' + str(atm_dry.tmp[-1]) + ' K,',
                    'dT_max = ' + str(dTglobal_dry) + ' K',
                )
